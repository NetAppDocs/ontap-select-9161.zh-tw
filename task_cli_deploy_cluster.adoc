...
.硬體 RAID
--
如果使用本機硬體 RAID 控制器，則必須在每個節點上至少建立一個資料儲存區 （ESX） 或一個link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool["儲存池（KVM）"]，用於系統資料以及根目錄和資料集合體。您必須在設定 ONTAP Select 節點時附加儲存資源池。

--
.軟體 RAID
--
如果使用軟體 RAID，則必須至少建立一個資料儲存區（ESX）或一個link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool["儲存池（KVM）"]用於儲存系統資料，並確保 SSD 磁碟機可用於根目錄和資料集合體。您必須在設定 ONTAP Select 節點時附加儲存資源池和磁碟。

--
====
. 可用的ONTAP Select版本
+
Deploy 管理實用程式僅包含一個版本的ONTAP Select。如果您要使用早期版本的ONTAP Select部署集群，則必須先link:task_cli_deploy_image_add.html["新增ONTAP Select映像"]到您的 Deploy 實例。

. 用於生產部署的ONTAP Select許可證
+
在生產環境中部署ONTAP Select叢集之前，您必須購買儲存容量許可證並下載相關的許可證文件。您可以link:concept_lic_production.html["在每個節點上授權存儲"]使用_Capacity Tiers_模型或使用_Capacity Pools_模型許可共享池。
...
.開始之前
使用以下方式執行網路連線檢查器link:task_adm_connectivity.html["網頁使用者介面"]或link:task_cli_connectivity.html["命令列介面"]確認內部網路上的叢集節點之間的連通性。
