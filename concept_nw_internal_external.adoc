...
NOTE: 與內部流量類似，外部流量可以在 vSwitch 層 (VST) 和外部交換器層 (EST) 進行標記。此外，外部流量也可以由ONTAP Select虛擬機本身透過稱為 VGT 的過程進行標記。請參閱link:concept_nw_data_mgmt_separation.html["數據和管理流量分離"]了解更多詳情。
