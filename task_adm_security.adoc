...
| 密碼 | 輸入關聯使用者名稱的密碼。 
|===
. 新增新的管理伺服器後，您可以選擇點擊image:icon_kebab.gif["選項"]並選擇以下選項之一：
+
** 更新憑證
** 驗證憑證
** 刪除管理伺服器
...
配置 YubiKey PIN 並按照以下步驟產生或匯入遠端支援代理 (RSA) 或橢圓曲線數位簽章演算法 (ECDSA) 私鑰和憑證link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647： ONTAP中的多因素身份驗證"^] 。
...
* 使用 SSH 相容格式匯出公鑰 `ssh-keygen -e` PIV 命令
